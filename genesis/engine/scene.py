...
class Scene(RBC):
    ...
    def add_mesh_light(
        ...
        if not isinstance(self.renderer_options, gs.renderers.RayTracer):
            ...
            elif isinstance(self.renderer_options, gs.renderers.ApolloRenderer):
                gs.raise_exception(
                    "This method is only supported by RayTracer. Please use 'add_light' when using ApolloRenderer."
                )
            else:
                gs.raise_exception(
    ...
    def add_light(
        ...
        """
        if not isinstance(self.renderer_options, gs.renderers.BatchRenderer) and not isinstance(
            self.renderer_options, gs.renderers.ApolloRenderer
        ):
            if isinstance(self.renderer_options, gs.renderers.RayTracer):
                gs.raise_exception(
