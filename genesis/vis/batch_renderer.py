...
class BatchRenderer(RBC):
    ...
    def render(self, rgb=True, depth=False, segmentation=False, normal=False, force_render=False):
        ...
        rgb_torch = [rgb_torch[i] for i in range(rgb_torch.shape[0])]
        depth_torch = [depth_torch[i] for i in range(depth_torch.shape[0])]

        self._rgb_torch = tuple(rgb_torch)
        self._depth_torch = tuple(depth_torch)
        return self._rgb_torch, self._depth_torch, None, None
